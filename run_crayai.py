...
parser.add_argument('--gens_per_epoch', default=1)
...
cmd = "python train.py %s --distributed cray --ranks-per-node 1 -v --gpu 0" % config_file
if args.run_pbt:
    cmd += " --load_checkpoint @checkpoint --save_checkpoint @checkpoint"

timestr = time.strftime("%Y%m%d-%H%M%S")
evaluator = hpo.Evaluator(cmd,
                          run_path="/lus/scratch/aheye/work/gnn/hpo/run%s" % timestr,
                          nodes=args.nodes,
                          launcher='wlm',
                          verbose=True, 
                          nodes_per_eval=args.nodes_per_eval,
                          checkpoint=args.checkpoint if args.run_pbt else '',
                          alloc_args="-J agnn-heptrkx --time=24:00:00 -C P100 --gres=gpu",
